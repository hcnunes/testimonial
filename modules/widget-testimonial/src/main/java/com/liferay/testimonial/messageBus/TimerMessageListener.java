...
@Component(immediate = true, property = { "cron.expression=0 0/10 * * * ?" }, service = TimerMessageListener.class)
public class TimerMessageListener extends BaseMessageListener {

	@Reference
	TestamentMessagingSender _testamentMessagingSender;

	@Reference
	private TestamentLocalService _testamentLocalService;

	@Reference
	private ContactLocalService _contactLocalSerice;
	...
	protected void doReceive(Message message) throws Exception {
		...
		Testament testament = _testamentLocalService.getTestament(2);
		Contact contact = _contactLocalSerice.getContact(testament.getContactId());
		...
		if(dueDate.before(today)) {
			String testamentTitle = testament.getTitle();
			String testamentMessage = testament.getMessage();
			String owner = testament.getUserName();
			String contactName = contact.getName();
			String contactEmail = contact.getEmail();
			
			
			_testamentMessagingSender.sendSomeMessage(testamentTitle, testamentMessage, 
					owner, contactName, contactEmail);
			
			System.out.println("TestamentMessagingSender called");
		}
